...
Pune, India \\
bhoiraditya7497@gmail.com}
\and
\IEEEauthorblockN{Omkar Ingle}
\IEEEauthorblockA{\textit{Department of Computer Engineering} \\
\textit{PES Modern College of Engineering}\\
Pune, India \\
omkar_ingle_comp@moderncoe.edu.in}
\and
\IEEEauthorblockN{Aftab Pathan}
\IEEEauthorblockA{\textit{Department of Computer Engineering} \\
\textit{PES Modern College of Engineering}\\
Pune, India \\
aftab_pathan_comp@moderncoe.edu.in}
\and
\IEEEauthorblockN{Abhishek Shinde}
\IEEEauthorblockA{\textit{Department of Computer Engineering} \\
\textit{PES Modern College of Engineering}\\
Pune, India \\
abhishek_shinde_comp@moderncoe.edu.in}
\and
\IEEEauthorblockN{Aryan Shinde}
\IEEEauthorblockA{\textit{Department of Computer Engineering} \\
\textit{PES Modern College of Engineering}\\
Pune, India \\
aryan_shinde_comp@moderncoe.edu.in}
\and
...
% ===== ABSTRACT =====
\begin{abstract}
Traditional electronic voting systems rely on centralized trust, lack independent verifiability, and offer limited ballot privacy. This paper presents \textbf{Bharat E-Vote}, a decentralized electronic voting platform built on Ethereum smart contracts with a hash-based commitment scheme for vote privacy and a six-layer defense-in-depth security architecture. Votes are recorded as immutable blockchain state transitions. A commitment scheme based on finite-field arithmetic hides voter choices, while a simplified proof-of-knowledge protocol provides a structural framework for on-chain verification (with limitations discussed in Section~IV-B). The security architecture combines bot protection with rate limiting, OTP-based multi-factor authentication, keystroke dynamics anomaly detection, smart contract role-based access control, and HTTP security header enforcement. An ERC-2771 meta-transaction forwarder~\cite{openzeppelin2024} enables gasless voting. The system was validated through 85 automated smart contract tests (100\% pass rate). Performance benchmarks show vote inclusion in blocks within 12--18 seconds on Ethereum Sepolia testnet (with probabilistic finality at approximately 12 minutes), and API response times below 150ms. We present an honest assessment of the system's limitations, including the proof-of-concept nature of the privacy module and the trust assumptions inherent in the architecture.
\end{abstract}
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Defense-in-Depth:} Six security layers ensure no single-point compromise defeats the system.
...
Bharat E-Vote uses a three-tier architecture: React frontend, Node.js/Express backend, and Ethereum blockchain. The blockchain smart contract is the final trust boundary for vote integrity, though off-chain components introduce additional trust assumptions (Section~VIII).
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{ZKPVoting.sol} (privacy mode): Commitment-based voting with nullifier-based double-vote prevention and a simplified proof-of-knowledge verification. \textbf{Important:} This contract stores commitments but does not perform on-chain tallying---vote counts are not incremented per-candidate. The privacy module serves as a verifiable ballot box, not a tallying system. Result determination in ZKP mode would require either homomorphic tallying over commitments (not possible with our SHA-256-wrapped scheme) or a trusted off-chain tally, which reintroduces the centralization problem. This tallying gap is a known limitation (Section~VII-D).
\end{itemize}
...
(1) Voter authenticates via backend MFA; (2) connects MetaMask wallet; (3) frontend invokes the appropriate contract via Ethers.js; (4) smart contract validates authorization and records the vote; (5) results are read from on-chain state.
...
The on-chain verifier implements a simplified check rather than a full Schnorr sigma protocol~\cite{schnorr1991}. The prover (browser) computes:
\begin{align}
...
The on-chain verifier checks: $H(C, N, s_v, s_r, n) \bmod p \stackrel{?}{=} e$.
...
\subsection{Event Obfuscation (Not Cryptographic Privacy)}
The \texttt{VoteCast} event emits $\text{keccak256}(\text{candidateId}, \text{timestamp}, \text{prevrandao})$. Since \texttt{timestamp} and \texttt{prevrandao} are public on-chain values and candidate IDs are drawn from a small set (typically 5--20), an observer can precompute all possible hashes and brute-force match in milliseconds. \textbf{This provides cosmetic obfuscation, not cryptographic privacy.} The \texttt{Voter} struct's exclusion of \texttt{votedCandidateId} prevents direct storage reads, but event analysis defeats this. True on-chain privacy requires not emitting any derivation of the candidate ID, or using a scheme where the search space is computationally infeasible to enumerate.

% ===== V. SECURITY ARCHITECTURE =====
\section{Six-Layer Defense-in-Depth Security}

The security architecture comprises six functionally distinct layers. Bot protection and rate limiting, though implemented via three distinct technologies (Cloudflare Turnstile, express-rate-limit, Upstash Redis), are grouped as a single perimeter layer as they implement the same control class.

\begin{table}[htbp]
\centering
\caption{Six-Layer Defence-in-Depth Architecture}
\label{tab:security}
...
\hline
2 & Password + OTP + JWT & Multi-factor identity verification with single-active-session enforcement \\
\hline
...
\hline
5 & Express Middleware & Payload size limiting (10KB), XSS input sanitization \\
\hline
...
\vspace{2pt}
\scriptsize{$^*$Keystroke dynamics is a \emph{monitoring} layer, not an enforcement layer. It flags anomalies but does not terminate sessions. See Section~VIII.}
\end{table}

\subsection{Multi-Factor Authentication}
Two-step MFA: Step~1 verifies password via \texttt{bcrypt} (cost factor 12), generates a 6-digit OTP (hashed, 5-minute expiry); Step~2 verifies OTP and issues JWT (20-minute expiry) with single-active-session enforcement.
...
Solidity version 0.8.19 (smart contracts), Hardhat (testing), Ethers.js v6 (blockchain interaction), Node.js/Express (backend), Prisma ORM over Supabase PostgreSQL (14 models), React 18/Vite (frontend), MetaMask (wallet).
...
\hline
\texttt{vote()} (first vote) & $\sim$85,000 & \$0.04 \\
\hline
\texttt{vote()} (re-vote) & $\sim$95,000 & \$0.05 \\
\hline
\texttt{authorizeVoter()} & $\sim$65,000 & \$0.03 \\
\hline
\texttt{submitEncryptedVote()} & $\sim$120,000 & \$0.06 \\
\hline
\texttt{addCandidate()} & $\sim$110,000 & \$0.06 \\
\hline
\end{tabular}

\vspace{2pt}
\scriptsize{$^*$At 10 gwei gas price and ETH=\$2,500. Mainnet costs vary significantly. At 50 gwei, costs are 5x higher. For 960M voters, total gas cost for \texttt{vote()} plus \texttt{authorizeVoter()} would be approximately \$60M--\$300M (\$0.04+\$0.03 per voter at 10 gwei, scaling linearly with gas price), making Layer-2 deployment essential for national scale.}
\end{table}
...
85 automated tests (Hardhat/Chai/Mocha) validated the following functional categories: deployment, access control, vote casting, re-voting, constituency matching, timeline enforcement, event privacy, and nullifier enforcement. VotingV2.sol: 72 tests; ZKPVoting.sol: 13 tests. All passed (100\% rate).
...
\hline
Info.\ Disc. & Vote reveal & Candidate ID excluded from struct; event obfuscation (limited---see Sec.~IV-D) \\
\hline
...
\subsection{Summary}
This paper presented Bharat E-Vote, a blockchain-based voting platform that achieves on-chain verifiability and multi-layered security. The system was validated through 85 automated tests (100\% pass rate) and met all performance targets.
