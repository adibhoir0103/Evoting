...
### Data Flow
1. **User authenticates** via Backend (OTP sent via Brevo API, checked against Supabase PostgreSQL)
...
|--------|------|-----------|-----------|-------------|
| POST | `/api/v1/auth/send-otp` | 10/5min + Upstash | ✅ | Send OTP via Brevo |
...
| L4: Authentication | Backend JWT + Redis + Postgres | Identity verification |
...
**Q: Why use a custom auth implementation instead of Supabase Auth or Clerk?**
A: Clerk's Passkey/WebAuthn features required a paid Pro subscription ($25/month). Supabase Auth is excellent, but building our own JWT+Redis+Brevo auth system demonstrated a deeper understanding of security engineering principles for the academic evaluation. Supabase is strictly used for PostgreSQL database hosting.
